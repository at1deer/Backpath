...
function requireV0CheckSupport(ctx) {
  ...
  if (manifest.operation !== "locate-loss") {
    ...
    for (const label of ["source", "target"]) {
      ...
      if (!equivalence || equivalence.mode !== "exact") {
        errors.push(`v0 check supports only ${label}.equivalence.mode='exact'`);
      }
    }
  }
  ...
  if (manifest.operation === "compare-paths") {
    for (const [pathIndex, pathSpec] of (manifest.paths || []).entries()) {
      ...
      if (!pathEquivalence || pathEquivalence.mode !== "exact") {
        errors.push(`v0 check supports only paths[${pathIndex}].target.equivalence.mode='exact'`);
      }
      for (const [edgeIndex, edge] of (pathSpec.edges || []).entries()) {
        const edgeEquivalence = edge.target && edge.target.equivalence;
        if (!edgeEquivalence || edgeEquivalence.mode !== "exact") {
          errors.push(`v0 check supports only paths[${pathIndex}].edges[${edgeIndex}].target.equivalence.mode='exact'`);
        }
      }
    }
  }
  ...
  if (manifest.operation === "locate-loss") {
    for (const [edgeIndex, edge] of (manifest.pipeline || []).entries()) {
      const edgeEquivalence = edge.target && edge.target.equivalence;
      if (!edgeEquivalence || edgeEquivalence.mode !== "exact") {
        errors.push(`v0 check supports only pipeline[${edgeIndex}].target.equivalence.mode='exact'`);
      }
    }
  }
