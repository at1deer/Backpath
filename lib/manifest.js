...
function requireV0CheckSupport(ctx) {
  ...
  if (manifest.operation === "locate-loss" && manifest.search && manifest.search.mode !== "corpus") {
    errors.push("v0.8 check supports locate-loss only with search.mode='corpus'");
  }

  if (manifest.memory && manifest.memory.mode === "declared") {
    if (manifest.operation !== "return-failure") {
      errors.push("v0.7 check supports memory.mode='declared' only for return-failure");
    }
    if (typeof manifest.memory.forwardTargetPath !== "string") {
      errors.push("v0.7 check requires memory.forwardTargetPath when memory.mode='declared'");
    }
    if (typeof manifest.memory.forwardMemoryPath !== "string") {
      errors.push("v0.7 check requires memory.forwardMemoryPath when memory.mode='declared'");
    }
    if (manifest.memory.reverseEnvelope === false) {
      errors.push("v0.7 check supports declared memory only with memory.reverseEnvelope=true");
    }
    if (manifest.memory.requireBinding === true) {
      errors.push("v0.7 check does not yet support memory.requireBinding=true");
    }
  }

  if (manifest.operation !== "locate-loss") {
    if (!manifest.search) {
      errors.push("v0.8 check requires search.mode");
    } else if (!SEARCH_MODES.has(manifest.search.mode)) {
      errors.push("v0.8 check supports search.mode='corpus', 'exhaustive', 'generate', or 'corpus-mutation'");
    } else if (manifest.search.mode === "exhaustive" && !(manifest.source && manifest.source.enumerator)) {
      errors.push("v0.8 check requires source.enumerator when search.mode='exhaustive'");
    } else if (manifest.search.mode === "generate" && !(manifest.source && manifest.source.generator)) {
      errors.push("v0.8 check requires source.generator when search.mode='generate'");
    } else if ((manifest.search.mode === "corpus" || manifest.search.mode === "corpus-mutation") && !(manifest.source && manifest.source.corpus)) {
      errors.push(`v0.8 check requires source.corpus when search.mode='${manifest.search.mode}'`);
    }
...
function validateDomain(value, pointer, errors) {
  ...
  if (Object.prototype.hasOwnProperty.call(value, "enumerator")) {
    validateCommand(value.enumerator, `${pointer}.enumerator`, errors);
  }
  if (Object.prototype.hasOwnProperty.call(value, "generator")) {
    validateCommand(value.generator, `${pointer}.generator`, errors);
  }
}
