...
const { exactEquivalent } = require("./equivalence");
...
function replayDistinctionCollision(witness, witnessPath) {
  ...
  const replayed =
    runA.succeeded &&
    runB.succeeded &&
    !exactEquivalent(sourceA, sourceB) &&
    exactEquivalent(runA.output, expectedA) &&
    exactEquivalent(runB.output, expectedB) &&
    exactEquivalent(runA.output, runB.output);
...
function replayReturnFailure(witness, witnessPath) {
  ...
  const reverseRun = forwardRun.succeeded
    ...
    : null;
  const replayed = replayedReturnClassification(witness, sourceValue, targetValue, returnedValue, forwardRun, reverseRun, manifestPath);
...
function replayDomainExclusion(witness, witnessPath) {
  ...
  const replayed =
    ...
    (classification === "invalid_target" && run.succeeded && invalidTargetReplay(run.output, expectedTarget, manifestPath));
...
function replayPathDivergence(witness, witnessPath) {
  ...
  const replayed = samePathDivergenceWitness(witness, divergence, pathResults);
...
function replayedReturnClassification(witness, sourceValue, targetValue, returnedValue, forwardRun, reverseRun, manifestPath) {
  if (witness.classification === "forward_excluded") {
    return !forwardRun.succeeded;
  }

  if (!forwardRun.succeeded || !exactEquivalent(forwardRun.output, targetValue) || reverseRun === null) {
    return false;
  }
  ...
  if (witness.classification === "invalid_return") {
    return reverseRun.succeeded && invalidSourceReplay(reverseRun.output, returnedValue, manifestPath);
  }
  if (witness.classification === "return_drift") {
    return (
      reverseRun.succeeded &&
      exactEquivalent(reverseRun.output, returnedValue) &&
      !exactEquivalent(sourceValue, reverseRun.output)
    );
  }
  return false;
}

function invalidTargetReplay(output, expectedTarget, manifestPath) {
  if (!exactEquivalent(output, expectedTarget)) {
    return false;
  }
...
function invalidSourceReplay(output, expectedReturn, manifestPath) {
  if (!exactEquivalent(output, expectedReturn)) {
    return false;
  }
  const validators = manifestValidators(manifestPath);
...
function samePathDivergenceWitness(witness, divergence, pathResults) {
  if (!divergence) {
    return false;
  }
  ...
  return pathResults
    ...
    .every((pathResult) => (
      ...
      exactEquivalent(pathResult.terminal.value, expectedTerminals.get(pathResult.name))
    ));
}
