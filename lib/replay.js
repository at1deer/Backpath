...
const { declaredMemorySpec, extractJsonPointer, reverseInputForMemory } = require("./memory");
...
function replayReturnFailure(witness, witnessPath) {
  ...
  const forwardResult = forwardRun.succeeded ? replayReturnForwardResult(ctx, validators, forwardRun.output) : null;
  let reverseRun = null;
  if (forwardResult && !forwardResult.classification) {
    const reverseCommand = commandFromSection(witness.reverse || {}, witnessPath, manifestPath);
    reverseRun = runJsonCommand(reverseCommand.command, forwardResult.reverseInput, reverseCommand.defaultCwd, reverseCommand.options);
  }
  const replayed = replayedReturnClassification(ctx, witness, sourceValue, targetValue, returnedValue, forwardRun, reverseRun, manifestPath, forwardResult);
...
function replayReturnForwardResult(ctx, validators, output) {
  const memorySpec = ctx && ctx.manifest ? declaredMemorySpec(ctx.manifest) : null;
  if (!memorySpec) {
    return replayDirectReturnForward(validators, output);
  }
  return replayDeclaredMemoryReturnForward(validators, output, memorySpec);
}

function replayDirectReturnForward(validators, output) {
  const targetValidation = validateWithCompiled(validators.target, output);
...
function replayDeclaredMemoryReturnForward(validators, envelope, memorySpec) {
  ...
  const targetValidation = validateWithCompiled(validators.target, targetExtraction.value);
  if (!targetValidation.valid) {
    return {
      classification: "forward_excluded",
      target: targetExtraction.value
    };
  }
  ...
  const memoryValidation = validateWithCompiled(validators.memory, memoryExtraction.value);
  if (!memoryValidation.valid) {
    return {
      classification: "retained_memory_failure",
      subtype: "invalid_memory",
      target: targetExtraction.value,
      memory: memoryExtraction.value
    };
  }
...
function replayedReturnClassification(ctx, witness, sourceValue, targetValue, returnedValue, forwardRun, reverseRun, manifestPath, forwardResult) {
  if (witness.classification === "forward_excluded") {
    return !forwardRun.succeeded || (forwardResult && forwardResult.classification === "forward_excluded");
  }

  if (witness.classification === "retained_memory_failure") {
    return (
      Boolean(forwardResult) &&
      forwardResult.classification === "retained_memory_failure" &&
      witnessDiagnosticSubtype(witness) === forwardResult.subtype
    );
  }

  if (
    !forwardRun.succeeded ||
    !forwardResult ||
    forwardResult.classification ||
    !targetEquivalent(ctx, forwardResult.target, targetValue) ||
    reverseRun === null
  ) {
    return false;
  }
...
function witnessDiagnosticSubtype(witness) {
  const diagnostic = Array.isArray(witness.diagnostics)
    ? witness.diagnostics.find((item) => item && item.subtype)
    : null;
  return diagnostic ? diagnostic.subtype : null;
}
...
function manifestValidators(manifestPath) {
  if (!manifestPath || !fs.existsSync(manifestPath)) {
    return { source: null, target: null, memory: null };
  }
  ...
  try {
    ...
    return {
      ...
      target: ctx.manifest.target && ctx.manifest.target.schema
        ...
        : null,
      memory: ctx.manifest.memory && ctx.manifest.memory.schema
        ? compileSchemaFile(path.resolve(ctx.baseDir, ctx.manifest.memory.schema))
        : null
    };
  } catch (_error) {
    return { source: null, target: null, memory: null };
