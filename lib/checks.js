...
function sourceDistinctPhrase(ctx) {
  return equivalenceMode(ctx.manifest.source && ctx.manifest.source.equivalence) === "exact"
    ? "source-distinct corpus items"
    : "corpus items distinct under the declared source equivalence";
}
...
function replayCountsForReturn(ctx, source, target, returned) {
  ...
  for (let index = 0; index < requestedForward; index += 1) {
    ...
    if (!run.succeeded || !targetEquivalent(ctx, run.output, target)) {
      break;
    }
    forwardReplays += 1;
  }
  ...
  for (let index = 0; index < requestedReverse; index += 1) {
    ...
    if (!run.succeeded || !sourceEquivalent(ctx, run.output, returned)) {
      break;
    }
    reverseReplays += 1;
  }
